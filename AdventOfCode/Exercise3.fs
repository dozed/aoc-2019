...
let parseDirString (str: string) =
    let d = charToDir str.[0]
    let num = int str.[1..]
    (d, num)

let parseDirsString (str: string) =
    List.map parseDirString (str.Split(',') |> Array.toList)

let add ((x, y): int * int) ((d, l): Dir * int) =
    match d with
    | Up -> (x, y + l)
    | Down -> (x, y - l)
    | Right -> (x + l, y)
...
let getSegments (str: string) =
    let xs = parseDirsString input1
    let coords = List.fold (fun (x :: xs) -> fun d -> (add x d) :: x :: xs) [ (0, 0) ] xs
    let segments = List.pairwise (List.rev coords)
    segments

let points ((x, y): int * int) ((d, l): Dir * int): (int * int) list =
    match d with
    | Up -> [for i in 1..l do (x, y + i)]
    | Down -> [for i in 1..l do (x, y - i)]
    | Right -> [for i in 1..l do (x + i, y)]
    | Left -> [for i in 1..l do (x - i, y)]

let distanceToOrigin (x:int) (y:int) =
    Math.Abs(x) + Math.Abs(y)

let exercise3 =
    // printfn "%A" (parseDirString "R1000")
    // let xs = parseDirsString input1
    // printfn "%A" xs

    // let xs = [| (Up, 3); (Right, 2) |] |> Array.toList
    // printfn "%A" (points (0,0) (Up, 5))
    // printfn "%A" coords
    // printfn "%A" coords2
    
    let xs1 = parseDirsString input1
    let xs2 = parseDirsString input2
    let coords1 = List.fold (fun ((x,y) :: xs) -> fun (d,l) -> List.append (List.rev (points (x,y) (d,l))) ((x,y) :: xs)) [(0, 0)] xs1 |> List.rev |> List.tail
    let coords2 = List.fold (fun ((x,y) :: xs) -> fun (d,l) -> List.append (List.rev (points (x,y) (d,l))) ((x,y) :: xs)) [(0, 0)] xs2 |> List.rev |> List.tail

    let coords1Set = Set.ofList coords1
    let coords2Set = Set.ofList coords2
    
    let common = Set.intersect coords1Set coords2Set |> Set.toList
    let commonWithDistance = List.map (fun (x,y) -> ((x,y), distanceToOrigin x y)) common
    let commonWithDistanceSorted = List.sortBy (fun ((x,y), d) -> d) commonWithDistance
    
    printfn "%A" commonWithDistanceSorted
    
    let distanceToPoint coords1 coords2 (x,y) =
        let n1 = (List.findIndex (fun (a,b) -> a = x && b = y) coords1) + 1  
        let n2 = (List.findIndex (fun (a,b) -> a = x && b = y) coords2) + 1
        n1 + n2

    let zs = List.map (fun (x,y) -> distanceToPoint coords1 coords2 (x,y)) common
    let zsSorted = List.sort zs
    printfn "%A" zs
    printfn "%A" zsSorted
